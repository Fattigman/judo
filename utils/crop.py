...
for i in range(rows):
    for j in range(cols):
        # Crop the image
        crop_img = img[i*cell_height:(i+1)*cell_height, j*cell_width:(j+1)*cell_width]
        # Save the cropped image
        cv2.imwrite(f'static/images/crop_{i}_{j}.png', crop_img)
        # Use pytesseract to recognize the text
        text = pytesseract.image_to_string(Image.open(f'static/images/crop_{i}_{j}.png')).replace(" ", "_").strip().replace("\n", "").lower()
